#!/usr/bin/env python3
from __future__ import annotations

import sys
from pathlib import Path
from typing import Any

from ruamel.yaml import YAML
from ruamel.yaml.comments import CommentedMap


yaml = YAML(typ="rt")
yaml.preserve_quotes = True
yaml.width = 10**9  # kein Re-Wrapping
yaml.indent(mapping=2, sequence=4, offset=2)
yaml.default_flow_style = False


ROOT_ORDER = [
    "openapi",
    "info",
    "servers",
    "externalDocs",
    "jsonSchemaDialect",
    "tags",
    "security",
    "paths",
    "webhooks",
    "components",
]

COMPONENTS_ORDER = [
    "schemas",
    "responses",
    "parameters",
    "examples",
    "requestBodies",
    "headers",
    "securitySchemes",
    "links",
    "callbacks",
    "pathItems",
]


def is_map(x: Any) -> bool:
    return isinstance(x, CommentedMap)


def reorder_map(src: CommentedMap, keys_in_order: list[str]) -> CommentedMap:
    out = CommentedMap()

    for k in keys_in_order:
        if k in src:
            out[k] = src[k]
            try:
                out.ca.items[k] = src.ca.items.get(k)
            except Exception:
                pass

    for k in src:
        if k not in out:
            out[k] = src[k]
            try:
                out.ca.items[k] = src.ca.items.get(k)
            except Exception:
                pass

    try:
        out.ca.comment = src.ca.comment
        out.ca.end = src.ca.end
    except Exception:
        pass

    return out


def sort_map_keys(src: Any) -> Any:
    if not is_map(src):
        return src
    return reorder_map(src, sorted(list(src.keys())))


def load_yaml(p: Path) -> CommentedMap:
    with p.open("r", encoding="utf-8") as f:
        doc = yaml.load(f)
    if not is_map(doc):
        raise SystemExit(f'ERROR: YAML root must be a mapping/object in "{p}".')
    return doc


def save_yaml(p: Path, doc: CommentedMap) -> None:
    with p.open("w", encoding="utf-8") as f:
        yaml.dump(doc, f)


def main(argv: list[str]) -> int:
    if len(argv) < 2:
        print(f"Usage: {argv[0]} <spec.yaml> [more.yaml ...]", file=sys.stderr)
        return 1

    rc = 0
    for fn in argv[1:]:
        p = Path(fn)
        try:
            doc = load_yaml(p)

            # Root reorder
            doc2 = reorder_map(doc, ROOT_ORDER)

            # components reorder + alphabetische Sortierung der Component-Objekte
            comps = doc2.get("components")
            if is_map(comps):
                comps2 = reorder_map(comps, COMPONENTS_ORDER)
                for section in COMPONENTS_ORDER:
                    if section in comps2 and is_map(comps2[section]):
                        comps2[section] = sort_map_keys(comps2[section])
                doc2["components"] = comps2

            save_yaml(p, doc2)
            print(f"Root and components sorted: {p}")
        except Exception as e:
            rc = 2
            print(f"ERR {p}: {e}", file=sys.stderr)

    return rc


if __name__ == "__main__":
    raise SystemExit(main(sys.argv))
