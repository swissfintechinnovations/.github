    ...
    return {
        Root: {
            enter(operation, { report, location, type }) {
                ...
                const missingKeys = sftiOpenapiSpecRef.filter(
                ...
                for (const key of missingKeys) {
                    report({
                        ...
                        location: location
                            ...
                            .key(),
                        suggest: [
                            `Include \`${key}\` object below \`${utils.get_common_prev_elem(
                                sftiOpenapiSpecRef,
                                keys,
                                key
                            )}\` object`,
                ...
                filteredKeys = keys.filter((item) =>
                    sftiOpenapiSpecRef.includes(item)
                );
                filteredRefs = sftiOpenapiSpecRef.filter((item) =>
                    keys.includes(item)
                );

                for (const key of filteredKeys) {
                    if (
                        filteredKeys.indexOf(key) !== filteredRefs.indexOf(key)
                    ) {
                        ...
                        report({
                            ...
                            suggest: [
                                `Include \`${key}\` object below \`${
                                    sftiOpenapiSpecRef[
                                        sftiOpenapiSpecRef.indexOf(key) - 1
                                    ]
                                }\` object`,
                            ],
                        });
